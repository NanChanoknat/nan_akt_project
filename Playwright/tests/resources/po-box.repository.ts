import axios from 'axios'

const tokenWMS = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..JOLRlmMKpSwaxnBb0mZJEm46VFO5XS-T5Du3mia5Z-M"

export async function createOnePOBox(token : string){
    const url = 'https://api.staging.akitahub.com/wms/boxes'
    const userToken = 'Bearer' + ' ' + token
    const headers = { Authorization : userToken }
    const boxPayload = {"boxes":[{"type":"pickout_box"}]}
    let response = 
        await axios.post(url,boxPayload,{headers}).then((response) => {
            // ถ้าทำงานสำเร็จ return PO Box code
            response = response.data.data[0].code
            return response
        })
        .catch((error) => {console.error(error)});
    
    let result = response
    return result
    
}

// createOnePOBox(tokenWMS)

export async function getLastPOBoxInfo(token : string){
    const lastPObox = await createOnePOBox(token)
    // console.log(lastPObox)
   
    const url = 'https://api.staging.akitahub.com/wms/boxes?type=pickout_box&page=1&limit=10&sort-by=code&order=asc'
    const userToken = 'Bearer' + ' ' + token
    const headers = { Authorization : userToken }
    // find last page
    let response = 
        await axios.get(url,{headers}).then((response) => {
            // ถ้าทำงานสำเร็จ return number of last page
            response = response.data.pages
            return response
        })
        .catch((error) => {
            // ถ้าทำงานไม่สำเร็จ เกิดข้อผิดพลาด
            console.error(error)
        });
    
    let result = response

    const lastBoxUrl = 'https://api.staging.akitahub.com/wms/boxes?type=pickout_box&page='+result+'&limit=10&sort-by=code&order=asc'

    let boxResponse = 
    await axios.get(lastBoxUrl,{headers}).then((boxResponse) => {
        // ถ้าทำงานสำเร็จ return number of last page
        boxResponse = boxResponse.data
        const num = boxResponse.data.length
        boxResponse = boxResponse.data[num - 1]
        return boxResponse
    })
    .catch((error) => {
        // ถ้าทำงานไม่สำเร็จ เกิดข้อผิดพลาด
        console.error(error)
    });

    let boxResult = boxResponse
    console.log(JSON.stringify(boxResult))
    return JSON.stringify(boxResult)
    
}

getLastPOBoxInfo(tokenWMS)
